 ...
 * @LastEditTime: 2019-11-11 09:21:11
...
<div>
    <div class="star">
        <p>明星大咖</p>
        <div class="star_main">
            <div v-for="(star,index) in stars" :key="index" @click="changetype(star.author,index)">
                <div class="star_img">
                    <img :src="star.authorImg">
                </div>
                <span class="star_author" :class="{star_author1:star.author==currauthor}">{{star.author}}</span>
            </div>
        </div>
    </div>
    <div class="star_box">
...
export default {
    ...
    created(){
        ...
        .then((response)=>{
            ...
            this.currauthor = this.stars[0].author;//把拿到的类型的第一个赋给当前类型
        })
        .catch((err)=>{
            console.log(err);
        });
